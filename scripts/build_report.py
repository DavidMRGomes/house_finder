...
const freguesiaOf=x=>{let parts=String(x.address||'').split(',').map(part=>part.trim()).filter(Boolean);return parts.length>=3?parts[0]:''};
...
['search','source','municipality','freguesia','sort','minPrice','maxPrice','hasBid'].forEach(id=>['input','change'].forEach(event=>document.getElementById(id).addEventListener(event,render)));
['marketSearch','marketSource','marketMunicipality','marketFreguesia','marketSort','marketMinPrice','marketMaxPrice'].forEach(id=>['input','change'].forEach(event=>document.getElementById(id).addEventListener(event,renderMarket)));
document.getElementById('heroImage').src=listings[0]?.image||'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=1200&q=85';render();renderSources();renderMarket();renderMarketStatus();
renderDiscovery();
</script>
</body></html>'''
