...
<section class="toolbar"><input id="search" type="search" placeholder="Search title, address or concelho"><select id="source"><option value="">All sources</option></select><select id="municipality" multiple size="3" aria-label="Filter by concelho"><option value="">All concelhos</option></select><select id="freguesia" multiple size="3" aria-label="Filter by freguesia"><option value="">All freguesias</option></select><select id="typology" multiple size="3" aria-label="Filter by typology"><option value="">All typologies</option></select><input id="minPrice" type="number" min="0" placeholder="Min €"><input id="maxPrice" type="number" min="0" placeholder="Max €"><label class="check"><input id="hasBid" type="checkbox"> Bid published</label><select id="sort"><option value="source">Sort by source</option><option value="price">Sort by published price</option><option value="title">Sort by title</option></select></section>
...
<section id="marketPanel" class="tab-panel hidden"><div class="source-panel"><h2>Homes for sale</h2><p class="hero p">These are ordinary-sale portals, kept separate from confiscated-property auctions. Open a portal to browse its current Lisbon inventory.</p><section class="toolbar"><input id="marketSearch" type="search" placeholder="Search title, address or concelho"><select id="marketSource"><option value="">All sources</option></select><select id="marketMunicipality" multiple size="3" aria-label="Filter by concelho"><option value="">All concelhos</option></select><select id="marketFreguesia" multiple size="3" aria-label="Filter by freguesia"><option value="">All freguesias</option></select><select id="marketTypology" multiple size="3" aria-label="Filter by typology"><option value="">All typologies</option></select><input id="marketMaxPrice" type="number" min="0" placeholder="Max €"><select id="marketSort"><option value="newest">Newest first</option><option value="price">Sort by price</option><option value="title">Sort by title</option><option value="source">Sort by source</option></select></section><div id="marketGrid" class="grid"></div><div class="market-status-panel"><h2>Market source results</h2><h3>Crawled successfully</h3><div id="marketSuccessful"></div><div id="marketUnsuccessful"></div></div></div></section>
</main>
...
const marketMunicipalitySelect=document.getElementById('marketMunicipality');[...new Set(marketListings.filter(x=>x.is_active!==0).map(x=>x.municipality).filter(Boolean))].sort().forEach(m=>marketMunicipalitySelect.insertAdjacentHTML('beforeend',`<option value="${esc(m)}">${esc(m)}</option>`));
...
const freguesiaOf=x=>{let parts=cleanedLocationParts(x), last=(parts.at(-1)||'').toLowerCase(), municipality=(x.municipality||'').toLowerCase();return parts.length>=3&&(last==='lisboa'||last===municipality)?parts.at(-3):''};
...
const marketFreguesiaSelect=document.getElementById('marketFreguesia');[...new Set(marketListings.filter(x=>x.is_active!==0).map(freguesiaOf).filter(Boolean))].sort().forEach(f=>marketFreguesiaSelect.insertAdjacentHTML('beforeend',`<option value="${esc(f)}">${esc(f)}</option>`));
...
function renderMarket(){let q=document.getElementById('marketSearch').value.toLowerCase(), source=marketSourceSelect.value, concelhos=[...marketMunicipalitySelect.selectedOptions].map(option=>option.value).filter(Boolean), freguesias=[...marketFreguesiaSelect.selectedOptions].map(option=>option.value).filter(Boolean), typologies=[...marketTypologySelect.selectedOptions].map(option=>option.value).filter(Boolean), sort=document.getElementById('marketSort').value, maxPrice=parseFloat(document.getElementById('marketMaxPrice').value);let rows=marketListings.filter(x=>{let price=x.published_price_eur;return x.is_active!==0&&(!q||[x.title,x.address,concelhoOf(x),x.source].join(' ').toLowerCase().includes(q))&&(!source||x.source===source)&&(!concelhos.length||concelhos.includes(concelhoOf(x)))&&(!freguesias.length||freguesias.includes(freguesiaOf(x)))&&(!typologies.length||typologies.includes(typologyOf(x)))&&(isNaN(maxPrice)||(price!=null&&price<=maxPrice))});rows.sort((a,b)=>sort==='title'?a.title.localeCompare(b.title):sort==='price'?((a.published_price_eur??Infinity)-(b.published_price_eur??Infinity)):sort==='source'?a.source.localeCompare(b.source):String(b.published_at||b.last_seen||'').localeCompare(String(a.published_at||a.last_seen||'')));let cards=rows.map(x=>`<article class="card"><div class="card-image"><img loading="lazy" src="${esc(x.image)}" alt="${esc(x.title)}"><span class="tag">${esc(x.source)}</span></div><div class="card-body"><h2>${esc(x.title)}</h2><div class="address">${esc(x.address||concelhoOf(x))}</div><div class="price-row"><div class="price"><label>Published price</label><strong>${euro(x.published_price_eur)}</strong></div><div class="price"><label>Published</label><strong>${esc((x.published_at||'').slice(0,10)||'Not published')}</strong></div></div></div><div class="card-foot"><span>Ordinary sale</span><a class="visit" href="${esc(x.url)}" target="_blank" rel="noreferrer">Open listing ↗</a></div></article>`);let portals=marketSources.map(x=>`<article class="market-card"><span class="market-status ${x.status==='blocked'?'blocked':''}">${esc(x.status||'reference')}</span><h3>${esc(x.name||x.source)}</h3><p>${esc(x.notes||x.error||'Market source')}</p><a href="${esc(x.url||'#')}" target="_blank" rel="noreferrer">Open Lisbon search ↗</a></article>`);document.getElementById('marketGrid').innerHTML=(cards.length?cards:['<div class="empty">No listings match this search.</div>']).concat(portals).join('')}
...
['search','source','municipality','freguesia','typology','sort','minPrice','maxPrice','hasBid'].forEach(id=>['input','change'].forEach(event=>document.getElementById(id).addEventListener(event,render)));
['marketSearch','marketSource','marketMunicipality','marketFreguesia','marketTypology','marketSort','marketMaxPrice'].forEach(id=>['input','change'].forEach(event=>document.getElementById(id).addEventListener(event,renderMarket)));
document.getElementById('heroImage').src=listings[0]?.image||'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=1200&q=85';render();renderSources();renderMarket();renderMarketStatus();renderMarketChanges();
renderDiscovery();
</script>
</body></html>'''
