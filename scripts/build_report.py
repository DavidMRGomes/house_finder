...
const marketTypologySelect=document.getElementById('marketTypology');[...new Set(marketListings.map(typologyOf).filter(Boolean))].sort().forEach(t=>marketTypologySelect.insertAdjacentHTML('beforeend',`<option value="${esc(t)}">${esc(t)}</option>`));
