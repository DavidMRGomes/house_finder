...
<section class="toolbar"><input id="search" type="search" placeholder="Search title, address or municipality"><select id="source"><option value="">All sources</option></select><select id="municipality"><option value="">All municipalities</option></select><input id="minPrice" type="number" min="0" placeholder="Min €"><input id="maxPrice" type="number" min="0" placeholder="Max €"><label class="check"><input id="hasBid" type="checkbox"> Bid published</label><select id="sort"><option value="source">Sort by source</option><option value="price">Sort by published price</option><option value="title">Sort by title</option></select></section>
...
<section id="marketPanel" class="tab-panel hidden"><div class="source-panel"><h2>Homes for sale</h2><p class="hero p">These are ordinary-sale portals, kept separate from confiscated-property auctions. Open a portal to browse its current Lisbon inventory.</p><div id="marketGrid" class="grid"></div></div></section>
</main>
...
function renderMarket(){let cards=marketListings.map(x=>`<article class="card"><div class="card-image"><img loading="lazy" src="${esc(x.image)}" alt="${esc(x.title)}"><span class="tag">${esc(x.source)}</span></div><div class="card-body"><h2>${esc(x.title)}</h2><div class="address">${esc(x.address||'Lisbon region')}</div><div class="price-row"><div class="price"><label>Published price</label><strong>${euro(x.published_price_eur)}</strong></div><div class="price"><label>Listing type</label><strong>For sale</strong></div></div></div><div class="card-foot"><span>Ordinary sale</span><a class="visit" href="${esc(x.url)}" target="_blank" rel="noreferrer">Open listing ↗</a></div></article>`);let portals=marketSources.map(x=>`<article class="market-card"><span class="market-status ${x.status==='blocked'?'blocked':''}">${esc(x.status||'reference')}</span><h3>${esc(x.name||x.source)}</h3><p>${esc(x.notes||x.error||'Market source')}</p><a href="${esc(x.url||'#')}" target="_blank" rel="noreferrer">Open Lisbon search ↗</a></article>`);document.getElementById('marketGrid').innerHTML=cards.concat(portals).join('')}
...
['search','source','municipality','sort','minPrice','maxPrice','hasBid'].forEach(id=>document.getElementById(id).addEventListener('input',render));document.getElementById('heroImage').src=listings[0]?.image||'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=1200&q=85';render();renderSources();renderMarket();
renderDiscovery();
</script>
</body></html>'''
